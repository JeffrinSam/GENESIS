        ...
        for key in keys:
            ...
            if key == "vlm_content":
                ...
                if self.model_type == "eagle":
                    import sys
                    print(f"DEBUG [Gr00tN1d6DataCollator]: Processing {len(values)} vlm_content items for Eagle", file=sys.stderr)
                    print(f"DEBUG [Gr00tN1d6DataCollator]: Conversations: {[v['conversation'] for v in values]}", file=sys.stderr)

                    image_inputs, _ = self.processor.process_vision_info(
                        [v["conversation"] for v in values]
                    )

                    print(f"DEBUG [Gr00tN1d6DataCollator]: process_vision_info returned {len(image_inputs) if image_inputs else 0} images", file=sys.stderr)
                    print(f"DEBUG [Gr00tN1d6DataCollator]: image_inputs type: {type(image_inputs)}", file=sys.stderr)
                    sys.stderr.flush()
                ...
                import sys
                print(f"DEBUG [Gr00tN1d6DataCollator]: processor() returned keys: {vlm_inputs.keys()}", file=sys.stderr)
                print(f"DEBUG [Gr00tN1d6DataCollator]: pixel_values in output: {'pixel_values' in vlm_inputs}", file=sys.stderr)
                sys.stderr.flush()
