...
class EagleBackbone(torch.nn.Module):
    def __init__(
        ...
        if model_name == "nvidia/Eagle-Block2A-2B-v2":
            # Note: Flash attention disabled for RTX 5090 compatibility
            ...
            # Load config with eager attention
            ...
            # Set on all config levels
            for sub_config in [config, getattr(config, 'text_config', None), getattr(config, 'vision_config', None)]:
                if sub_config is not None:
                    sub_config._attn_implementation = 'eager'
                    # Also set the raw attn_implementation attribute
                    if hasattr(sub_config, 'attn_implementation'):
                        sub_config.attn_implementation = 'eager'

            print(f"[Eagle] Config attn: main={config._attn_implementation}, vision={getattr(config.vision_config, '_attn_implementation', 'N/A')}, text={getattr(config.text_config, '_attn_implementation', 'N/A')}")
