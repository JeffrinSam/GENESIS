...
# Import from transformers with compatibility for older versions
try:
    ...
    # Fallback for transformers < 4.56 (docstrings not available)
    from transformers.image_processing_utils_fast import (
        BaseImageProcessorFast,
        DefaultFastImageProcessorKwargs,
        divide_to_patches,
...
# Backwards compatibility for transformers < 4.56
...
try:
    ...
    VideoInput = Union[List[Image.Image], List[np.ndarray], List[torch.Tensor], List[List[Image.Image]], List[List[np.ndarray]], List[List[torch.Tensor]]]

# make_batched_videos function
try:
    from transformers.image_utils import make_batched_videos
except ImportError:
    def make_batched_videos(videos):
        """Fallback implementation for make_batched_videos."""
...
class Eagle3_VLImageProcessorFast(BaseImageProcessorFast):
    ...
    def _preprocess(
        ...
        disable_grouping: bool = False,  # Add disable_grouping parameter for RTX 5090 compatibility
    ) -> BatchFeature:
        ...
        grouped_images, grouped_images_index = group_images_by_shape(images, disable_grouping)
